...
(function (AS) {
  ...
  function buildChalet() {
    ...
    /* Pignon : un prisme triangulaire obtenu d'un cylindre à trois faces
       couché, ce qui évite d'écrire une géométrie à la main. */
    var pignon = new THREE.CylinderGeometry(2.45, 2.45, 5.0, 3, 1);
    pignon.rotateX(Math.PI / 2);
    pignon.rotateZ(Math.PI / 6);
    parts.push({ geo: pignon, color: BOIS, matrix: at(0, 4.62, 0) });

    /* Toit : deux plans épais posés en pente, débordant largement. La neige
       dessus est ce qui le rend alpin plutôt que générique. */
    var pente = 0.62;
    for (var c = -1; c <= 1; c += 2) {
      var m = new THREE.Matrix4();
      m.compose(
        new THREE.Vector3(c * 2.05, 5.62, 0),
        new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, -c * pente)),
        new THREE.Vector3(1, 1, 1)
      );
      parts.push({ geo: box(5.2, 0.26, 6.2), color: CADRE, matrix: m });
      var mn = new THREE.Matrix4();
      mn.compose(
        new THREE.Vector3(c * 2.02, 5.82, 0),
        new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, -c * pente)),
        new THREE.Vector3(1, 1, 1)
      );
      parts.push({ geo: box(5.3, 0.22, 6.3), color: PAL.snow, matrix: mn });
    }

    /* Cheminée, et sa couronne de neige. */
    parts.push({ geo: box(0.85, 2.5, 0.85), color: PIERRE, matrix: at(-2.0, 6.4, -1.3) });
    parts.push({ geo: box(1.0, 0.2, 1.0), color: PAL.snow, matrix: at(-2.0, 7.7, -1.3) });
