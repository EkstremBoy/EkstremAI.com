...
const GRID_SIZE = 20;
// Reduced to 3 segments as requested
const INITIAL_SNAKE = [
...
export default function SnakeGame() {
    ...
    const wallHitRef = useRef(0); // For wall-grace logic
    ...
    useEffect(() => {
        ...
        const moveSnake = () => {
            ...
            setSnake(prevSnake => {
                ...
                // GRACE PERIOD ON WALLS
                const isOOB = newHead.x < 0 || newHead.x >= GRID_SIZE || newHead.y < 0 || newHead.y >= GRID_SIZE;
                if (isOOB) {
                    if (wallHitRef.current === 0) {
                        wallHitRef.current = 1; // Mark that we hit once
                        return prevSnake; // Pause movement for one tick
                    } else {
                        handleGameOver();
                        return prevSnake;
                    }
                ...
                // If we moved into a safe zone, reset grace
                wallHitRef.current = 0;
        ...
        // ACCELERATION - Slower start (200ms)
        const speed = Math.max(40, 200 - (Math.floor(score / 5) * 15) - (score % 5) * 2);
    ...
    // Story
    useEffect(() => {
        if (score === 20 && !hasSeen20Msg) { setShowStory(true); setHasSeen20Msg(true); }
        if (score === 25 && !hasSeen25Msg) { setShowStory(true); setHasSeen25Msg(true); }
    ...
    useEffect(() => {
        ...
        if (mode === 'RETRO') {
            ctx.fillStyle = '#8b956d'; ctx.fillRect(0, 0, canvas.width, canvas.height);
        ...
        // Draw Food
        if (mode === 'RETRO') {
            ctx.fillStyle = '#bc1e1e'; ctx.beginPath(); ctx.arc(food.x * cellSize + cellSize / 2, food.y * cellSize + cellSize / 2, cellSize / 2.5, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#4a2d1a'; ctx.fillRect(food.x * cellSize + cellSize / 2 - 1, food.y * cellSize + 2, 2, 4);
        } else {
            ctx.fillStyle = '#00f2ff'; ctx.globalAlpha = 0.6; ctx.beginPath(); ctx.arc(food.x * cellSize + cellSize / 2, food.y * cellSize + cellSize / 2, cellSize / 3, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1.0;
        }
        ...
        specialItems.forEach(item => {
            ...
            if (item.type === 'bonus') {
                if (mode === 'RETRO') {
                    ctx.fillStyle = '#ffd700'; ctx.shadowBlur = 10; ctx.shadowColor = 'gold'; ctx.beginPath(); ctx.arc(item.x * cellSize + cellSize / 2, item.y * cellSize + cellSize / 2, cellSize / 2.2, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
                } else {
                    ctx.shadowBlur = 30; ctx.shadowColor = '#00f2ff'; ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(item.x * cellSize + cellSize / 2, item.y * cellSize + cellSize / 2, cellSize / 2.5, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
                }
            } else if (item.type === 'negative') {
                if (mode === 'RETRO') {
                    ...
                    ctx.fillStyle = '#ff2d55'; ctx.globalAlpha = 0.4; ctx.fillRect(item.x * cellSize + 4, item.y * cellSize + 4, cellSize - 8, cellSize - 8);
                    ctx.strokeStyle = '#ff2d55'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(item.x * cellSize + 2, item.y * cellSize + 2); ctx.lineTo(item.x * cellSize + cellSize - 2, item.y * cellSize + cellSize - 2); ctx.stroke(); ctx.globalAlpha = 1.0;
                }
            }
        });
        ...
        snake.forEach((segment, i) => {
            const isHead = i === 0;
            if (mode === 'RETRO') {
                ctx.fillStyle = isHead ? '#0a0b08' : '#2a2d1f'; ctx.fillRect(segment.x * cellSize + 1, segment.y * cellSize + 1, cellSize - 2, cellSize - 2);
                if (isHead) {
                ...
                ctx.beginPath(); ctx.roundRect(segment.x * cellSize + 2, segment.y * cellSize + 2, cellSize - 4, cellSize - 4, isHead ? 6 : 3); ctx.fill(); ctx.shadowBlur = 0;
            }
        });
        ...
        <div className="flex flex-col items-center gap-8 w-full max-w-4xl">
            ...
            <div className="relative group">
                ...
                <AnimatePresence>
                    {!isPlaying && !isGameOver && (
                        ...
                        >
                            <div className="p-6 rounded-full bg-brand-cyan/20 border border-brand-cyan/30 text-brand-cyan mb-4 group-hover:scale-110 transition-transform"><Gamepad2 size={48} /></div>
                    ...
                    {isGameOver && (
                        ...
                        >
                            ...
                            <button onClick={startGame} className="flex items-center gap-2 px-8 py-4 rounded-2xl bg-brand-cyan font-black text-brand-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all"><RefreshCw size={20} /> Replay</button>
                        </motion.div>
                    )}
                    {showStory && (
            ...
            <div className="w-full glass p-8 rounded-[40px] border border-white/5">
                <div className="flex items-center gap-3 mb-8">
                    <Trophy className="text-brand-cyan" size={24} /><h3 className="text-xl font-black text-white uppercase tracking-widest italic">{t('leaderboard.title')}</h3>
                </div>
